...
@pytest.mark.asyncio
async def test_backup_download_maps_inspection_no_space_to_507(monkeypatch, tmp_path):
    local_archive = {}
    ...
    monkeypatch.setattr(
        ...
        lambda _archive_path: (_ for _ in ()).throw(no_space_error),
    )
...
@pytest.mark.asyncio
async def test_backup_to_webdav_maps_inspection_no_space_to_507_and_cleans_temp(
    monkeypatch, tmp_path
):
    local_archive = {}
    no_space_error = OSError(errno.ENOSPC, "No space left")
    ...
    async def fake_run_in_threadpool(func, *args):
        ...
        if func is getattr(system, "_inspect_generated_backup_archive", None):
            return func(*args)
        if func in {system.upload_file, system.prune_backups}:
            raise AssertionError(f"{func.__name__} should not run after inspection failure")
        raise AssertionError(f"unexpected threadpool function: {func}")
